...
d1.setAttribute("id", "center");
d1.style.display = "flex";
d1.style.flexDirection = "column";
d1.style.alignItems = "center";
d1.style.justifyContent = "space-between";
d1.style.height = "700px";
document.body.appendChild(d1);

let d2 = document.createElement("div");
d2.setAttribute("id", "data");
d2.style.textAlign = "center";
document.getElementById('center').appendChild(d2);

let d3 = document.createElement("div");
d3.setAttribute("id", "pagination");
d3.style.display = "inline-block";
document.getElementById('center').appendChild(d3);

let first = createPagination(1, "First");
document.getElementById('pagination').appendChild(first);

let prev = createPagination("prev", "Previous");
document.getElementById('pagination').appendChild(prev);

for (i = 2; i < totalPage; i++) {
    let a = createPagination(i, i);
    document.getElementById('pagination').appendChild(a);
}

let next = createPagination("next", "Next");
document.getElementById('pagination').appendChild(next);

let last = createPagination(totalPage, "Last");
document.getElementById('pagination').appendChild(last);

function createPagination(id, value) {
    ...
    a.style.transition = "background-color .3s";
...
function getData(e) {
    if ((e == "prev") && (currentIndex != 1)) {
        currentIndex -= 1;
    } else if ((e == "next") && (currentIndex != totalPage)) {
        currentIndex += 1;
    } else {
        currentIndex = Number(e);
    }

    startIndex = currentIndex == 1 ? 0 : (currentIndex - 1) * 10;
...
function populateData(arr) {
    ...
    let tbl = "<table border cellpadding='15' width='550px' style='height:600px; border: 5px solid #3D9BE2';>" +
        "<tr>" +
        "<th>ID</th>" +
        "<th>NAME</th>" +
        "<th>EMAIL</th>" +
